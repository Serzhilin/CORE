...
## Status: token + base-style batches done. Card, Button, Input, Select, Badge, Modal, ProgressBar, EmojiBadge, EmojiPicker, CollapsiblePanel, Panel, Avatar, Label, Icon/TrashIcon, MenuItem, Table/Thead/Th/Td + topbar-slot-row layout primitives done. Input divergence resolved (unified to `.input` look). Dead animation/prose CSS deleted. Subagent completeness audit (2026-07-13) found 4 items, all 4 now resolved (see below). Post-audit gap list identified (Avatar, Label, Icon, Dropdown/Menu, Table, Tabs, Checkbox, Toast, Loading, Typography, Layout shell) — extracting one at a time; user approved extracting all remaining items in sequence. Avatar, Label, Icon, MenuItem, Table, Tabs, Loading done. Checkbox/Radio/Switch and Toast/alert/banner investigated and excluded (nothing in CORE clears the extraction bar — see rationale below). Next: Typography scale.
