...
## Status: token + base-style batches done. Card, Button, Input, Select, Badge, Modal, ProgressBar, EmojiBadge, EmojiPicker, CollapsiblePanel, Panel, Avatar, Label, Icon/TrashIcon, MenuItem, Table/Thead/Th/Td + topbar-slot-row layout primitives done. Input divergence resolved (unified to `.input` look). Dead animation/prose CSS deleted. Subagent completeness audit (2026-07-13) found 4 items, all 4 now resolved (see below). Post-audit gap list identified (Avatar, Label, Icon, Dropdown/Menu, Table, Tabs, Checkbox, Toast, Loading, Typography, Layout shell) — extracting one at a time; user approved extracting all remaining items in sequence. Avatar, Label, Icon, MenuItem, Table, Tabs, Loading, Typography scale (Heading/SectionLabel/ErrorText), Page shell done. Checkbox/Radio/Switch and Toast/alert/banner investigated and excluded (nothing in CORE clears the extraction bar — see rationale below). Post-audit gap list fully closed — nothing left to extract.
...
- [x] `Tabs` (tab bar — flex row, bottom border track, active underline+color+weight) — found hand-rolled in `AdminPanel.jsx` (top-level admin nav) and `admin/WorkgroupsTab.jsx` (nested per-workgroup members/roles/details tabs), with measured divergence between the two, not hypothetical
  - Created `ecommons-ui/src/components/Tabs.tsx` (ecommons-ui commit `a735a17`), no dedicated CSS (pure inline-style). Props: `tabs: {key, label}[]`, `activeKey`, `onChange`, plus explicit overrides for every value that actually diverged between the two real call sites — `activeColor` (default `var(--color-terracotta)`), `borderWidth` (default `2`), `fontSize` (default `'0.95rem'`), `activeFontWeight` (default `700`), `gap` (default `4`), `textTransform` (default `'none'`). Active tab gets a `${borderWidth}px solid ${activeColor}` bottom border and `marginBottom: -borderWidth` to sit flush on the track; inactive tabs get a transparent border of the same width so height doesn't shift.
  - Swapped `AdminPanel.jsx`'s top-level tab bar to `<Tabs tabs={visibleTabs} activeKey={activeKey} onChange={(key) => navigate(\`/admin/${key}\`)} />` — every prop matches the component's defaults exactly (terracotta/2px/0.95rem/700/4px/none), so no overrides needed there.
  - Swapped `admin/WorkgroupsTab.jsx`'s nested tab bar to `<Tabs tabs={[...]} activeKey={getTab(wg.id)} onChange={...} activeColor={wg.color} borderWidth={1} fontSize="0.85rem" activeFontWeight={600} gap={0} textTransform="capitalize" />` — this site's original inline styles used a 1px border, 600 active weight, 0.85rem font, no gap, `text-transform: capitalize`, and a dynamic per-workgroup `activeColor` instead of the static terracotta, all preserved via explicit props.
  - No exclusions — both known tab-bar sites in CORE were in scope and swapped.
  - CORE commit `feed0a2`. Verified `vite build` clean.
- [ ] `Checkbox`/`Radio`/`Switch` — **nothing extracted, documented as excluded.** Grepped all of CORE: exactly one `type="checkbox"` exists (`OrganogramView.jsx`'s "Show unavailable" filter toggle, `<label style={checkStyle}><input type="checkbox".../> Show unavailable</label>`), zero `type="radio"`, zero custom switch/toggle UI anywhere. That single checkbox is a bare native browser checkbox with a flex label wrapper — no custom skin, no neubrutalist border/box treatment, nothing matching this app's design language to encapsulate. One site, no visual identity, no sibling Radio/Switch to pair with — doesn't clear the extraction bar (same bar that excluded `OrganogramView`'s checkbox label back when auditing `Label`). Left as-is in CORE.
- [ ] `Toast`/alert/banner — **nothing extracted, documented as excluded.** Grepped all of CORE for toast/snackbar/alert-banner/notification patterns: none exist. Error messaging is either a native `alert()` JS dialog or plain inline colored text (e.g. `<div style={{fontSize:'0.8rem', color:'var(--color-red)'}}>{error}</div>` in `CommunityTab.jsx`/`SuperadminPage.jsx`/`W3dsLinkCard.jsx`) — no box, no icon, no dismiss, no overlay/positioning. Not the same UI concept as a toast/banner, so not force-fit into this slot. (The inline-error-text shape is itself a candidate for the Typography-scale item, not this one.)
- [x] `Loading` (colored placeholder text shown during data/route loading) — found duplicated with the same `color: var(--color-charcoal-light)` shape across 6 sites in 4 files, differing only in wrapper layout and fallback copy
  - Created `ecommons-ui/src/components/Loading.tsx` (ecommons-ui commit `d8d0251`), no dedicated CSS (pure inline-style). Props: `children` (default `'Loading…'`), `style` — base style always sets `color: var(--color-charcoal-light)`, caller `style` spread last for layout overrides.
  - Swapped 6 sites: `App.jsx` (x3 — full-viewport centered loading gate, route-`Suspense` fallback with default text, superadmin-route `Suspense` fallback with `padding: 32`), `views/SuperadminPage.jsx` (1, identical full-viewport gate, byte-identical to `App.jsx`'s), `views/OrganogramView.jsx` (1, plain no-override case), `views/graph/GraphView.jsx` (1, `padding: 40` override + custom `Loading graph…` text).
  - **Excluded from scope:** `OrganogramView.jsx`'s second `Suspense` fallback (`<div>Loading graph…</div>`, no color styling at all) — a different, uncolored shape, not part of this duplicated pattern.
  - CORE commit `008586a`. Verified `vite build` clean.
- [x] `Heading`/`SectionLabel`/`ErrorText` (Typography scale) — a backgrounded Explore subagent surveyed all page/section titles and error-text blocks in CORE; three genuine shapes emerged: a page-title style (`font-family: var(--font-title), font-weight 700, font-size 2rem`) duplicated across every `useSetTopBarSlot` call, a muted-uppercase section-label style (`text-transform uppercase, letter-spacing 0.05em, color var(--color-charcoal-light)`) duplicated across ~15 `h3`/`h4`/`div` sites at varying font sizes, and a red inline error-text style (`color var(--color-red)`) duplicated across ~10 sites. The much larger "muted helper text" pattern (`color: var(--color-charcoal-light)` at continuously varying font sizes, no canonical value) was explicitly investigated and rejected — no dominant size, would just be an unhelpful passthrough wrapper, not a real design-system component.
  - Created `ecommons-ui/src/components/Heading.tsx`, `SectionLabel.tsx`, `ErrorText.tsx` (ecommons-ui commit `7e0dc73`), no dedicated CSS (pure inline-style). All three take an `as` prop to preserve each call site's original HTML tag (semantics/accessibility unaffected by the visual extraction):
    - `Heading`: `as?: 'span' | 'h1'` (default `'span'`), `fontSize` (default `'2rem'`), `fontWeight` (default `700`), `color` (default `var(--color-charcoal)` — confirmed to already be the global `body` color default in `tokens/index.css`, so omitting it at a call site is pixel-identical), `style`.
    - `SectionLabel`: `as?: 'h3' | 'h4' | 'div'` (default `'h3'`), `fontSize` (default `'1rem'`), `fontWeight` (optional — only applied if explicitly passed, since most sites rely on the browser's default `h3`/`h4` boldness rather than an explicit value), `letterSpacing` (default `'0.05em'`), always forces `textTransform: 'uppercase'` and `color: var(--color-charcoal-light)`, `style`.
    - `ErrorText`: `as?: 'div' | 'p'` (default `'div'`), `fontSize` (default `'0.8rem'`), always forces `color: var(--color-red)`, `style`.
  - Swapped every real site across 15 files: `MyProfile.jsx`, `AdminPanel.jsx`, `MyWorkgroups.jsx`, `MyAvailability.jsx` (Heading page titles + 1 SectionLabel), `LoginScreen.jsx` (Heading `as="h1"` + ErrorText `as="p"`), `OnboardingScreen.jsx` (Heading `as="h1"`), `SuperadminPage.jsx` (1 SectionLabel + 1 ErrorText), `admin/AvailabilityTab.jsx` (3 SectionLabel, one `as="h4" fontSize="0.85rem"`), `W3dsLinkCard.jsx` (1 SectionLabel + 2 ErrorText), `admin/CommunityTab.jsx` (4 SectionLabel incl. 2 `as="h4" fontSize="0.78rem"` + 3 ErrorText), `PersonModal.jsx` (1 SectionLabel `as="h4"`), `admin/WorkgroupsTab.jsx` (2 SectionLabel `as="h4"`), `InfoPanel.jsx` (3 SectionLabel `as="div" fontWeight={700}`, div-tier variant with explicit weight since these are plain `<div>`s with no browser-default boldness), `TopBar.jsx` (1 SectionLabel `as="div" fontWeight={700} letterSpacing="0.06em"`, a near-duplicate variant with different letter-spacing/padding), `admin/MembersTab.jsx` (1 ErrorText `as="p" fontSize="0.85rem"`).
  - CORE commit `b96bb22`. Verified `vite build` clean across all 15 files.
- [x] `Page` (routed-view content wrapper — `maxWidth` cap + `margin: '0 auto'` centering) — found in every view rendered inside `App.jsx`'s `<main>`, varying only in `maxWidth` (600/520/680/620/680/1100/560)
  - Created `ecommons-ui/src/components/Page.tsx` (ecommons-ui commit `636069a`), no dedicated CSS (pure inline-style). Single required prop `maxWidth: number`, plus `style` spread last for any extra per-site layout (e.g. `admin/AvailabilityTab.jsx`'s `display:'flex', flexDirection:'column', gap:24` stacking its two Cards).
  - Swapped 7 sites: `MyProfile.jsx` (600), `MyAvailability.jsx` (520), `MyWorkgroups.jsx` (620), `admin/AvailabilityTab.jsx` (680, + flex/gap style), `admin/WorkgroupsTab.jsx` (680), `admin/MembersTab.jsx` (1100), `admin/CommunityTab.jsx` (560).
  - **Excluded from scope:** `SuperadminPage.jsx`'s wrapper (`maxWidth: 720, margin: '0 auto', padding: 32, fontFamily: 'var(--font-sans)'`) — a different context, not the same shape. `SuperadminPage` is a standalone route rendered outside `App.jsx`'s `<main>` (see `App.jsx`'s top-level `<Routes>`), so its wrapper also has to supply the `padding`/`fontFamily` that `<main>` normally provides — a single, structurally different site, not measured duplication of the `Page` shape. Also excluded: `App.jsx`'s own `<main>`/`TopBar` shell — a single CORE-specific layout site wiring `TopBarSlotProvider`/`TopBar`, not a duplicated pattern.
  - CORE commit `36e8330`. Verified `vite build` clean across all 7 files.

This closes the post-audit gap list — every item has been either extracted or investigated and excluded with documented rationale.
